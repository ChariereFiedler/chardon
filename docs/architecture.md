...
│  notify.ts          PreToolUse   Bash   → toil alert on stdout (opt-in)      │
└────────────────────────────┬───────────────────────────────────────────────┘
                             ▼ (import ../lib/*.js)
...
│  schema.sql   idempotent DDL (6 tables)                                    │
└────────────────────────────┬───────────────────────────────────────────────┘
                             ▼
...
1. **SessionStart** → `session-start` inserts a `sessions` row (id, repo, branch,
   ticket via `ticketRegex`, type main/worktree).
...
3. **PreToolUse Bash** → `notify` (when `CHARDON_ACTIVE=1`) reads the DB and alerts on
   stdout on a toil loop. It writes nothing.
...
`schema.sql` creates 6 tables (idempotent). Five are live in v1; two are defined for
future use.
...
**Regressions** (a previously-fixed friction whose count climbed back — `detectRegressions`)
and **Cross-project candidates** (a Bash command recurring across ≥ `CROSS_REPO_MIN` repos —
...
- **Retention is manual**: history is bounded by the `/chardon-purge` command
  (`retentionDays`, default 90) — there is no automatic scheduled purge yet.
...
- **Hooks are silent by design**: fail-open emits no error log — a collection bug is
...
26 files, 203 tests (Vitest). Hooks are tested as **real subprocesses**
(`node --experimental-strip-types`, stdin + env) — fail-open enforced by tests.
